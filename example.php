<?php
/**
 * Contoh Script Bridge API Dreamshort via StreamAPI.web.id
 * ----------------------------------------------------
...
// Fungsi bantuan buat manggil endpoint Dreamshort di sistem Bridge
function callDreamshortAPI($urlTujuan, $token) {
...
// UJI COBA KODE
// -----------------------------------------------------------------
$baseUrl = "https://streamapi.web.id/p/dreamshort/api/v1";

// Contoh 1: Fitur Cari Film Dreamshort
echo "\n=====================[ 1. Pencarian Dreamshort ]==================\n";
$kataKunci = urlencode("cinta");
$halaman = 1;
$urlCari = "{$baseUrl}/search/{$kataKunci}/{$halaman}?lang=in";

$hasilCari = callDreamshortAPI($urlCari, $API_TOKEN);
if ($hasilCari) {
...
$hasilDetail = callDreamshortAPI($urlDetail, $API_TOKEN);
if ($hasilDetail) {
...
$hasilNonton = callDreamshortAPI($urlNonton, $API_TOKEN);
if ($hasilNonton) {
...
echo "▶▶▶ https://streamapi.web.id/docs/dreamshort ◀◀◀\n\n";
